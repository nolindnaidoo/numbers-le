...
const CASES: readonly Case[] = [
	{
		...
		build: () =>
			JSON.stringify(
				Array.from({ length: 40_000 }, (_, i) => ({ id: i, value: i * 1.5, count: i })),
				null,
				1,
			),
	},
	{
		...
		arg: 'csv',
		build: () =>
			`a,b,c\n${Array.from({ length: 50_000 }, (_, i) => `${i},${i * 2.5},${i * 7}`).join('\n')}`,
	},
	{
		label: 'INI config',
		arg: 'ini',
		build: () =>
			Array.from({ length: 30_000 }, (_, i) => `key${i}=${i * 3}`).join('\n'),
	},
];

async function run(content: string, c: Case): Promise<number> {
	return extractNumber(content, c.arg as never, `bench.${c.arg}`).numbers.length;
}
