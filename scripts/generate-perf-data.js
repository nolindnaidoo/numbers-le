...
function generateJsonData(targetBytes) {
    const records = [];
...
function generateCsvData(targetBytes) {
    const headers = [
        "id",
        "name",
        "email",
...
function generateData(format, targetBytes) {
    switch (format) {
        case "json":
...
function runBenchmarks() {
    ...
    try {
        const output = (0, node_child_process_1.execSync)("bun test ./src/extraction/performance.bench.ts --test-timeout=60000", { encoding: "utf-8", cwd: process.cwd() });
...
function extractPerformanceReport(benchmarkOutput) {
...
function updatePerformanceMd(report) {
    console.log("📝 Updating docs/PERFORMANCE.md...");
    const perfMdPath = (0, node_path_1.join)(process.cwd(), "docs", "PERFORMANCE.md");
    (0, node_fs_1.writeFileSync)(perfMdPath, report, "utf-8");
    console.log("✅ docs/PERFORMANCE.md updated!\n");
}
function updateReadmeTable(benchmarkOutput) {
...
`.trim();
    ...
    const readmePath = (0, node_path_1.join)(process.cwd(), "README.md");
    let readme = (0, node_fs_1.readFileSync)(readmePath, "utf-8");
    ...
    if (startIdx !== -1 && endIdx !== -1) {
        ...
        (0, node_fs_1.writeFileSync)(readmePath, readme, "utf-8");
...
function generateReadmeRows(metrics) {
    const rows = [];
    // JSON rows
    if (metrics.JSON) {
        for (const m of metrics.JSON.slice(0, 3)) {
            const throughput = m.throughput.replace(/,/g, "");
            rows.push(`| **JSON** | ${m.size} | ${throughput} | ~${m.time} | < 1MB  | Apple Silicon |`);
        }
    }
    // CSV rows
    if (metrics.CSV) {
    ...
    if (metrics.ENV && metrics.ENV[0]) {
        ...
        rows.push(`| **ENV**  | 5K lines   | ${m.throughput} | ~${m.time} | < 1MB  | Apple Silicon |`);
    }
    return rows.join("\n");
}
function main() {
    ...
    const perfDir = (0, node_path_1.join)(process.cwd(), "src", "extraction", "__performance__");
    for (const spec of FILE_SPECS) {
        const filePath = (0, node_path_1.join)(perfDir, spec.name);
...
main();
//# sourceMappingURL=generate-perf-data.js.map