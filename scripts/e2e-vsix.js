...
exports.run = async function run() {
	...
	// Smoke the primary command end to end. File type detection reads
	// the filename extension, so the fixture must be a real .json file.
	const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nle-e2e-'));
	...
	const result = vscode.workspace.textDocuments.find(
		(d) => d.languageId === 'plaintext' && d.getText().startsWith('1\\n2.5'),
	);
