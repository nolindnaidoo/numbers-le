 ...
 * nothing, then reported "Extracted N numbers" — a count for results the user
...
const BIG = `{"values":[${Array.from({ length: 150 }, (_, i) => i).join(',')}]}`;

async function runExtract(events: string[]): Promise<void> {
	await extractNumbers(makeDeps(events));
}
...
describe('large output: "Copy only"', () => {
	it('copies even when the automatic copy setting is off', async () => {
		const events: string[] = [];
		_setConfig('numbers-le.copyToClipboardEnabled', false);
		chooseCopyOnly();
