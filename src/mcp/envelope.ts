 ...
 * `ok` is not `success`.
 *
 * `extractNumber` never fails on an unknown format — it falls back to a plain
 * text scan — so an empty result is a true empty result. `ok` is driven by
 * whether a parser actually reported a problem.
 */
...
export function toDiagnostics(result: ExtractionResult): readonly Diagnostic[] {
	return result.errors.map((e) => ({
		// Every ParseError this engine produces is a genuine failure to read the
		// document; there is no warning channel to preserve.
