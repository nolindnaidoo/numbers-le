...
interface PerformanceMetrics {
	...
	readonly throughputNumbersPerSecond: number;
...
const PERFORMANCE_FILES = Object.freeze([
	...
	{ name: '100k.toml', format: 'toml' },
	{ name: '50k.yaml', format: 'yaml' },
	{ name: '25k.ini', format: 'ini' },
] as const);
...
function runSinglePerformanceTest(
	...
	extractNumbers(content.slice(0, 1000), format, fileName);
	...
	const result = extractNumbers(content, format, fileName);

	const endTime = performance.now();
	const memoryAfter = measureMemoryUsage();

	const extractionTimeMs = Math.round((endTime - startTime) * 100) / 100;
	const extractedCount = result.success ? result.numbers.length : 0;
	const throughputNumbersPerSecond = Math.round(
...
function runPerformanceTests(): PerformanceResult {
	console.log('🚀 Starting Numbers-LE Performance Tests\n');
	...
	for (const { name, format } of PERFORMANCE_FILES) {
		...
		try {
			...
			console.log(
				`  ✓ Extracted ${metric.extractedCount.toLocaleString()} numbers`,
			);
			console.log(
				`  ✓ Throughput: ${metric.throughputNumbersPerSecond.toLocaleString()} numbers/sec`,
			);
			console.log('');
		} catch (error) {
...
function formatPerformanceReport(result: PerformanceResult): string {
	...
	let report = '# Numbers-LE Performance Test Results\n\n';
	...
	for (const metric of metrics) {
		...
		report += `| ${metric.formatType.toUpperCase()} | ${
			...
			metric.extractionTimeMs
		} | ${metric.extractedCount.toLocaleString()} | ${metric.throughputNumbersPerSecond.toLocaleString()} | ${
	...
	for (const [format, formatResults] of formatMetrics) {
		...
		report += `${Math.round(
			avgExtracted,
		).toLocaleString()} numbers extracted on average.\n\n`;
	}
