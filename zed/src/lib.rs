//! The Zed side of Numbers-LE.
//!
//! This crate holds no extraction logic. It resolves `numbers-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx numbers-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "numbers-le-mcp";
const SERVER_PATH: &str = "node_modules/numbers-le-mcp/server.js";

struct NumbersLEExtension;
...
impl zed::Extension for NumbersLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(NumbersLEExtension);
