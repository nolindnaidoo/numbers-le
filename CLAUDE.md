...
- **What the contract holds equal is the shared `extract_numbers` MCP tool**,
...
- **How a number is printed is part of that contract.** The corpus pins the
  boundaries of JavaScript's `Number::toString`, because the crate has to
  reproduce them and Rust's own formatting does not.
